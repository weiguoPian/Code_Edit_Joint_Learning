        ...
        for _ in tqdm(bar):
            ...
            if args.do_eval and ((global_step + 1) % args.eval_steps == 0) and eval_flag:
                ...
                for batch in tqdm(eval_dataloader):
                    ...
                    with torch.no_grad():
                        if args.localization:
                            _, loss, num, location_logits = model(source_ids=source_ids, source_mask=source_mask,
                                                                target_ids=target_ids, target_mask=target_mask)
                ...
                for batch in tqdm(eval_dataloader):
                    ...
                    with torch.no_grad():
                        if args.localization:
                            ...
                            preds, location_logits = model(source_ids=source_ids, source_mask=source_mask)
                            location_logits = F.softmax(location_logits, dim=-1)
                            all_location_logits = torch.cat((all_location_logits, location_logits.detach().cpu()), dim=0)
                        else:
